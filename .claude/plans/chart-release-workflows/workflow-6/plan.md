...
**Approach**: Single rolling PR that accumulates releases.

**Why one PR instead of per-chart?**
- Simpler to manage and review
- Natural accumulation of commits from main
- One merge releases all pending charts
- Reduces PR noise

**How it works**:
1. Chart A merges to main → W6 creates PR "release: chartA-v1.0.0"
2. Chart B merges to main → W6 updates PR title/body to include chartB
3. Reviewer merges PR → All tagged charts are released (triggers W7/W8)

**Code**:
```bash
# Collect all tags created in this run
CREATED_TAGS="$TAG_NAME"  # or accumulate from loop
...
| Chart | Version | Tag |
|-------|---------|-----|
| chartA | 1.0.0 | chartA-v1.0.0 |
| chartB | 2.0.0 | chartB-v2.0.0 |
...
**A**: **Single rolling PR**. When multiple charts merge to main over time:
1. First chart → Creates PR "release: chartA-v1.0.0"
2. Second chart → Updates same PR to "release: chartA-v1.0.0, chartB-v2.0.0"
3. Reviewer merges → All accumulated charts released together

Benefits:
- Simpler to manage and review
- Natural batching as main accumulates commits
- Reduces PR noise
- One merge triggers W7/W8 for all pending releases
